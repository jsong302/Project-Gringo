...
export async function handleAdmin(
  ...
  if (trimmed === '' || trimmed === 'help') {
    await respondEphemeral(respond, [
      ...
      '',
      '*Content Queue*',
      '- `show content queue` — view upcoming lessons and lunfardo in the queue',
      '- `fill queue for 2 weeks` — pre-generate content for upcoming days',
      '- `show queue item <N>` — view full details of a queued item',
      '- `edit queue item <N>` — modify a queued lesson or lunfardo post',
      '- `reorder queue item <N> to <date>` — move an item to a different date',
      '- `remove queue item <N>` — archive a queued item',
      '- `regenerate queue item <N>` — re-generate content via LLM',
      '',
      '*Exit Exams*',
